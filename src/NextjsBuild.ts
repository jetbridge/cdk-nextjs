...
export class NextjsBuild extends Construct {
  ...
  private validatePaths() {
    ...
    if (!fs.existsSync(nextjsPath)) {
      throw new Error(
        `Invalid nextjsPath ${nextjsPath} - directory does not exist at "${path.resolve(nextjsPath)}"`,
      );
    }
    ...
    const packageJson = JSON.parse(
      fs.readFileSync(path.join(nextjsPath, "package.json"), "utf8"),
    );
    if (!packageJson.scripts || !packageJson.scripts.build) {
      throw new Error(
        `No "build" script found within package.json in "${nextjsPath}".`,
      );
    }
  }
  ...
  private build() {
    ...
    const buildCommand =
      this.props.buildCommand ?? `npx @opennextjs/aws@^3 build`;
    // run build
    if (!this.props.quiet) {
  ...
  readPublicFileList() {
    ...
    return listDirectory(this.nextStaticDir).map((file) =>
      path.join("/", path.relative(this.nextStaticDir, file)),
    );
  }
  ...
  private assertBuildDirExists(throwIfMissing = true) {
    ...
    if (!fs.existsSync(dir)) {
      if (throwIfMissing) {
        throw new Error(
          `Build directory "${dir}" does not exist. Try removing skipBuild: true option.`,
        );
      }
      return false;
    }
  ...
  public get openNextOutput(): OpenNextOutput | undefined {
    ...
    if (!fs.existsSync(outputPath)) {
      if (!this.props.quiet) {
        console.warn(
          `Warning: open-next.output.json not found at ${outputPath}`,
        );
      }
      return undefined;
    }
    ...
    try {
      ...
      if (validation.warnings.length > 0 && !this.props.quiet) {
        console.warn(
          `open-next.output.json warnings: ${validation.warnings.join(", ")}`,
        );
      }
      ...
      for (const [name, origin] of Object.entries(output.origins)) {
        if (origin.type === "function" && origin.bundle) {
          ...
          if (!fs.existsSync(bundlePath)) {
            if (!this.props.quiet) {
              console.warn(
                `Warning: Bundle path does not exist: ${bundlePath}`,
              );
            }
          ...
          if (!stats.isDirectory()) {
            if (!this.props.quiet) {
              console.warn(
                `Warning: Bundle path is not a directory: ${bundlePath}`,
              );
            }
            continue;
          }
    ...
    try {
      ...
      if (serverFunction && fs.existsSync(serverFunction.bundlePath)) {
        ...
        if (stats.isDirectory()) {
          ...
          if (!this.props.quiet) {
            console.warn(
              `Bundle path is not a directory: ${serverFunction.bundlePath}`,
            );
          }
        }
      }
